  ...
  while (resultsIndex < 5) {
    ...
    if (tempPhrase[tpal-1] == "Search" && tempPhrase[tpal-2] == "Google") {
      if (tempPhrase[0] == "who" || tempPhrase[0] == "Who" || tempPhrase[0] == "WHO" ||
          ...
          if (pplength > 30) {
            thepassphrase = thepassphrase.slice(0, 30);
          }
...
button.onclick = async function() {
  bottomSectionOutput = "<br><b>Your Passphrase results ranked from strongest to weakest:</b><br><br>";
  ...
  function outputHTML() {
    if (passphraseArray[0] != null && passphraseArray[0].pIDB != "") {
      ...
      for (var j = 0; j < 5; j++) {
        ...
        var itemNo = j + 1;
        if (passphraseArray[j].entropy > 60) bottomSectionOutput = bottomSectionOutput + itemNo + ": <b><i>" + passphraseArray[j].pass + "</b></i> has entropy: <b><i> " + passphraseArray[j].entropy.toFixed(2)+ "</b></i>" + passphraseArray[j].pIDB  + "<br><br>";
      }
...
async function dbcheck (data) {
  ...
  ref.orderByChild("password").equalTo(pphrase).once("value",snapshot => {
    ...
    if (userData){
      ...
      passphraseArray[data].pIDB = " and is present in the RockYou leaked password list. We do <u>not</u> recommend this passphrase.";
    } else {
